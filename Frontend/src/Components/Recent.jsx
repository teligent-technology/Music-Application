...
const Recents = () => {
  ...
  const [currentIndex, setCurrentIndex] = useState(null); // index of playing song
  const [isPlaying, setIsPlaying] = useState(false);
  const [progress, setProgress] = useState(0); // 0 to 1
  const audioRef = useRef(null);
  const progressRef = useRef(null);

  // Load recent songs from localStorage and map to Songs array
  useEffect(() => {
  ...
  const togglePlayPause = () => {
    if (!audioRef.current) {
      // No audio loaded - play first song if any
